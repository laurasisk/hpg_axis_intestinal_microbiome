...
duodenum_map <- duodenum_map_all [1:154,]
head(duodenum_map)
#since I took one of the samples out due to low sequence depth so I could have a higher rarefaction number:
...
feces_map <- feces_map_week10[1:77,]
head(feces_map)
...
#Also get rid of the cohort 2 and 3 samples
cecum_SVs <- cecum_SVs_all
cecum_map <- cecum_map_all
head(cecum_SVs)
...
#get rid of the controls
#Also get rid of the cohort 2 and 3 samples
ileum_SVs <- ileum_SVs_all
ileum_map <- ileum_map_all
head(ileum_SVs)
...
#get rid of the controls
#Also get rid of the cohort 2 and 3 samples
duodenum_SVs <- duodenum_SVs_all
duodenum_map <- duodenum_map_all
head(duodenum_SVs)
...
#get rid of the controls
#Also get rid of the cohort 2 and 3 samples
duodenum_genus <- duodenum_genus_all
head(duodenum_genus)
...
#get rid of the controls
#Also get rid of the cohort 2 and 3 samples
duodenum_family <- duodenum_family_all
...
```{r}
#cecum
cecum_unweighted_filtered_c45 <- read.csv("./data/cecum/unifrac_data/unweighted_unifrac_cecum.csv", header = TRUE, row.names = 1)

#ileum
ileum_unweighted_filtered_c45 <- read.csv("./data/ileum/unifrac_data/unweighted_unifrac_ileum.csv", header = TRUE, row.names = 1)


#duodenum
duodenum_unweighted_filtered_c45 <- read.csv("./data/duodenum/unifrac_data/unweighted_unifrac_duodenum.csv", header = TRUE, row.names = 1)
...
## 0.5  Lists for sample subsetting later
```{r}
...
### 3.3.4 Heatmaps with glucuronidase/BSH genera
All Envs
```{r}

glucuronidase_BSH_genera_dot <- gsub("-", "\\.", glucuronidase_BSH_genera)
BSH_genera_dot <- gsub("-", "\\.", BSH_genera)
glucuronidase_genera_dot <- gsub("-", "\\.", glucuronidase_genera)

intestine_genus_tx_group_by_group_glucuronidase_BSH_stack <- intestine_genus_tx_group_by_group_sample_stack[(intestine_genus_tx_group_by_group_sample_stack$Genus %in% glucuronidase_BSH_genera_dot ),]

intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Glucuronidase <- with(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack, ifelse(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Genus %in% c(glucuronidase_genera_dot), "Gluc.", ""))

intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$BSH <-  with(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack, ifelse(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Genus %in% c(BSH_genera_dot), "BSH", ""))

intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Enzyme <- paste(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Glucuronidase,intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$BSH)

intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Enzyme<- gsub("Gluc. BSH", "Glucuronidase and BSH", intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Enzyme)
intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Enzyme <- factor(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Enzyme, levels = c("Gluc. ", "Glucuronidase and BSH" ," BSH" ))

length(unique(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack$Genus))
```
```{r}
p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH <- ggplot(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack, aes(Group, Genus, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(axis.text = element_text(face="bold"))
p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH

ggsave('./results/environmental_analysis/all_envs/heatmap/genus/heatmap_genus_sample_types_facet_sample_type_glucuronidase_BSH.png',width=35, height=11, plot = p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH)

```
With enzyme labels
```{r}
#with enzyme labels 
p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH_labels <- ggplot(intestine_genus_tx_group_by_group_glucuronidase_BSH_stack, aes(x = Group, y = Genus, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
   theme(text = element_text(size = 30))  +  
  facet_grid(Enzyme~Sample_Type_Env, scales = "free", space = "free_y") +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1,)) +
  theme(axis.text = element_text(face="bold")) 
p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH_labels

ggsave('./results/environmental_analysis/all_envs/heatmap/genus/heatmap_genus_sample_types_facet_sample_type_glucuronidase_BSH_labels.png',width=35, height=15, plot = p_heatmap_genus_sample_type_env_facet_sample_type_glucuronidase_BSH_labels)
```

### 3.3.5 Heatmaps with glucuronidase/BSH SVs labeled by genera and enzyme
All Envs
```{r}
# select SVs that in genera with glucuronidase or BSH
intestine_glucuronidase_BSH_SVs <- filter(intestine_SV_taxonomy, Genus %in% glucuronidase_BSH_genera)


intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack <- intestine_SVs_tx_group_by_group_sample_stack[(intestine_SVs_tx_group_by_group_sample_stack$SVs %in% row.names(t(data.frame(t(intestine_glucuronidase_BSH_SVs))))),]

intestine_SV_taxonomy_X <-data.frame(t(data.frame(t(intestine_SV_taxonomy))))

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus <- intestine_SV_taxonomy_X$Genus[match(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$SVs, row.names(intestine_SV_taxonomy_X))]

glucuronidase_BSH_genera_dot <- gsub("-", "\\.", glucuronidase_BSH_genera)
BSH_genera_dot <- gsub("-", "\\.", BSH_genera)
glucuronidase_genera_dot <- gsub("-", "\\.", glucuronidase_genera)

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Glucuronidase <- with(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, ifelse(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus %in% c(glucuronidase_genera), "Gluc.", ""))

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$BSH <- with(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, ifelse(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus %in% c(BSH_genera), "BSH", ""))

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme <- paste(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Glucuronidase, intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$BSH)
  

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme<- gsub("Gluc. BSH", "Glucuronidase and BSH", intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme)
intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme <- factor(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme, levels = c("Gluc. ", "Glucuronidase and BSH" ," BSH" ))
length(unique(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus))
```


```{r}
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH <- ggplot(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(axis.text = element_text(face="bold"))
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH

ggsave('./results/environmental_analysis/all_envs/heatmap/SVs/heatmap_SVs_sample_types_facet_sample_type_glucuronidase_BSH.png',width=35, height=20, plot = p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH)

```

With enzyme labels
```{r}
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme <- ggplot(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(axis.text = element_text(face="bold"))
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme

ggsave('./results/environmental_analysis/all_envs/heatmap/SVs/heatmap_SVs_sample_types_facet_sample_type_glucuronidase_BSH_enzyme.png',width=35, height=20, plot = p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme)

```

With enzyme labels and genus
```{r}
intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full <-intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full <-gsub("Gluc. ", "Glucuronidase ", intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full)

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full <- factor(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full, levels =  c("Glucuronidase ", "Glucuronidase and BSH" ," BSH" ))

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus_abrev <-intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus_abrev <-gsub("Allorhizobium-Neorhizobium-Pararhizobium-Rhizobium", "A-N-P-R Genus", intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus_abrev)

intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus_abrev <-gsub("Burkholderia-Caballeronia-Paraburkholderia", "B-C-P Genus", intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Genus_abrev)
```
 

```{r}
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus2 <-ggplot(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
  theme(axis.text.y=element_blank()) +
   theme(text = element_text(size = 30))  +  
  facet_grid(Genus_abrev ~Sample_Type_Env, scales = "free", space = "free_y", switch = "y") +
  theme(strip.text.y.left = element_text(angle = 0)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(axis.text = element_text(face="bold")) 
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus2

ggsave('./results/environmental_analysis/all_envs/heatmap/SVs/heatmap_SVs_sample_types_facet_sample_type_glucuronidase_BSH_enzyme_genus2.png',width=30, height=30, plot = p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus2)

```


```{r}
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus <- ggplot(intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack, aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  #scale_fill_distiller(palette = "YlGnBu") +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_classic() +
  theme(axis.text.y=element_blank()) +
   theme(text = element_text(size = 30))  +  
  facet_grid(Enzyme_full + Genus_abrev ~Sample_Type_Env, scales = "free", space = "free_y", switch = "y") +
  theme(strip.text.y.left = element_text(angle = 0)) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +
  theme(axis.text = element_text(face="bold")) 
p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus

ggsave('./results/environmental_analysis/all_envs/heatmap/SVs/heatmap_SVs_sample_types_facet_sample_type_glucuronidase_BSH_enzyme_genus.png',width=34, height=30, plot = p_heatmap_SVs_sample_type_env_facet_sample_type_glucuronidase_BSH_enzyme_genus)
...
# 5. Figures
## 5.1 Supplemental Figures 1/2/3/4
### 5.1.1 Supplemental Figure 1
```{r}
...
p_faith_duodenum_SVs_fig <- ggplot(data = duodenum_alpha) +
  ...
  theme_alpha_plot() +
  labs(title = "Duodenum") +
  scale_x_discrete(labels=c("F_Mut" ,"F_WT", "M_Mut", "M_WT")) +
  facet_grid(~Sample_Type) +
  ylim(0,8) 
...
supplemental_figure_2 <- plot_grid(p_shannon_duodenum_SVs_fig,
                                   p_shannon_ileum_SVs_fig,
                                   p_shannon_cecum_SVs_fig,
                                   legend_groups_side,
...
ggsave("./results/environmental_analysis/figures/supplemental_figure_2.png",supplemental_figure_2, width = 12, height = 6)

ggsave("./results/environmental_analysis/figures/supplemental_figure_2.svg",supplemental_figure_2, width = 12, height = 6)
```
### 5.1.3 Supplemental Figure 3
```{r}
...
ggsave("./results/environmental_analysis/figures/supplemental_figure_3.png",supplemental_figure_3, width = 13, height = 7)

ggsave("./results/environmental_analysis/figures/supplemental_figure_3.svg",supplemental_figure_3, width = 13, height = 7)
```

### 5.1.4 Supplemental Figure 4
...
p_shannon_intestine_SVs_group_2 <- ggplot(data = intestine_alpha) +
  ...
  facet_grid(~Section) +
      theme(axis.title.x=element_blank(),
        axis.text.x=element_blank(),
        axis.ticks.x=element_blank())
p_shannon_intestine_SVs_group_2
...
Fig 2
```{r}
figure_2_AB <- plot_grid(p_NMDS_all_SVs_clr_c45_all_envs_sections_sample_group_2, 
...
figure_2_CDE <- plot_grid( p_shannon_intestine_SVs_group_2, 
                      p_faith_intestine_SVs_group_2, 
                      p_NMDS_all_family_clr_c45_all_envs_sections_sample_no_feces_2,
                      labels = c('C', 'D', 'E'), label_size = 12 ,
                  rel_widths = c(9, 12,18),
                  ncol = 3, nrow = 1)

figure_2_FGH <- plot_grid( p_duodenum_family_mean_both_cohorts_flip_genotype, 
          p_ileum_family_mean_both_cohorts_flip_genotype, 
          p_cecum_family_mean_both_cohorts_flip_genotype,
...
## 5.5  Supplemental Figure 5 Coefficient plots all Coefficients 
Theme
```{r}
...
## 5.6 Supplemental Figure 6 Heatmaps- WT sex differences
WT Differences by section
```{r}
...
## 5.6 Figure 5 Heatmap of gluc BSH
Break the stacked SV data into enzyme
```{r}
intestine_glucuronidase_glucuronidase_only_SVs_stack <- intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack[intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full=="Glucuronidase ", ]

intestine_glucuronidase_glucuronidase_BSH_SVs_stack <- intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack[intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full=="Glucuronidase and BSH", ]

intestine_glucuronidase_BSH_only_SVs_stack <- intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack[intestine_glucuronidase_BSH_SVs_tx_group_by_group_sample_stack$Enzyme_full==" BSH", ]
```
Theme
```{r}
theme_gluc_BSH_heatmap <- function(){ 
    font <- "Arial"   
    theme_classic() %+replace%    #replace elements 
    theme( 
...
Plot
```{r}
p_heatmap_SVs_glucuronidase_only <-ggplot(intestine_glucuronidase_glucuronidase_only_SVs_stack, 
  aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_gluc_BSH_heatmap() +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  facet_grid(Genus_abrev ~Sample_Type_Env, scales = "free", space = "free_y", switch = "y") 
p_heatmap_SVs_glucuronidase_only

p_heatmap_SVs_glucuronidase_BSH <-ggplot(intestine_glucuronidase_glucuronidase_BSH_SVs_stack, 
  aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_gluc_BSH_heatmap() +
  ...
  facet_grid(Genus_abrev ~Sample_Type_Env, scales = "free", space = "free_y", switch = "y") 
p_heatmap_SVs_glucuronidase_BSH


p_heatmap_SVs_BSH_only <- ggplot(intestine_glucuronidase_BSH_only_SVs_stack, 
  aes(Group, SVs, fill= CLR_Abundance)) + 
  geom_tile() +
  scale_fill_viridis_c(option = "D", begin = 0.1) +
  theme_gluc_BSH_heatmap() + facet_grid(
    Genus_abrev ~Sample_Type_Env, scales = "free", space = "free_y", switch = "y") +
  theme(axis.text.x = element_text(angle = 60, vjust = 1, hjust=1),
        strip.text.x = element_blank())
p_heatmap_SVs_BSH_only
```

Figure
```{r}
figure_5 <- plot_grid(p_heatmap_SVs_glucuronidase_only+ theme(legend.position='none'), 
                      p_heatmap_SVs_glucuronidase_BSH,
                      p_heatmap_SVs_BSH_only+ theme(legend.position='none'),  
                labels =  c( 'A', 'B', 'C'),
                     label_size = 12, 
                     align = "v", axis = "lr",
...
ggsave("./results/environmental_analysis/figures/figure_5.png",figure_5, width = 11, height = 14)

ggsave("./results/environmental_analysis/figures/figure_5.svg",figure_5, width = 11, height = 14)
```
